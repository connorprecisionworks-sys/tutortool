drop function if exists confirm_public_service_booking(text, uuid, timestamptz, text, text, text);
drop function if exists get_public_service_slots(text, uuid, date);
drop function if exists get_public_service(text, uuid);

-- Restores get_public_tutor_profile to its pre-C3 shape (no `id` on each
-- service entry) — the B4 version this migration replaced.
