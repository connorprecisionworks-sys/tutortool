-- D4: invoice line-item descriptions ("Session on Jul 05, 2026") are
-- generated server-side, not rendered from a raw column, so the app-wide
-- M/D/Y sweep needs to reach into these two functions too — the app-layer
-- date helper (lib/date.ts) can't touch text already baked into a stored
-- invoice_line_items.description. Every other line is unchanged from each
-- function's latest prior version (q5's create_draft_invoice, q4's
-- update_session) — only the to_char format argument moves from
-- 'Mon DD, YYYY' to 'FMMM/FMDD/YYYY' (the FM prefix suppresses to_char's
-- default zero-padding, so July 5th reads "7/5/2026" not "07/05/2026").
